...
function fractalWidth(u, iterations = 6) {
    let width = 1;
    let amplitude = 0.6;  // How "wavy" the largest ripples are
    let frequency = 3.1;

    for (let i = 0; i < iterations; i++) {
        width += amplitude * Math.sin(frequency * u * Math.PI * 2);
        amplitude *= 0.5;   // Each layer is half as tall
        frequency *= 2.4;   // Each layer oscillates faster (non-integer to avoid repetition)
    }

    return Math.max(width, 0.1); // Prevent negative/zero width
}

function fractalDisplacement(u, v, iterations = 5) {
    let displacement = 0;
    let amplitude = 1.0;
    let frequency = 1.0;

    for (let i = 0; i < iterations; i++) {
        // Use both u and v to create 2D surface patterns
        displacement += amplitude * Math.sin(frequency * u * Math.PI * 2) * Math.cos(frequency * v * Math.PI * 3.7);
        displacement += amplitude * Math.cos(frequency * u * Math.PI * 3.1) * Math.sin(frequency * v * Math.PI * 2.3);

        amplitude *= 0.5;  // Each layer contributes less
        frequency *= 2.2;  // Each layer has finer detail
    }
...
function mobiusStrip(u, v, target) {
    ...
    const R = 20;
    //const w = fractalWidth(u / (Math.PI * 2), 6);
    const w = 1;
    ...
    // Calculate normal vector (approximate, good enough for displacement)
    // The normal points "outward" from the surface
    const normalX = Math.cos(u) * Math.cos(u / 2);
    const normalY = Math.sin(u) * Math.cos(u / 2);
    const normalZ = Math.sin(u / 2);
    ...
    const displacementAmount = fractalDisplacement(u / (Math.PI * 2), v / w) * 0.3;

    target.set(
        x + nx * displacementAmount,
        y + ny * displacementAmount,
        z + nz * displacementAmount
    );
}
